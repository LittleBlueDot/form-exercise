...
export class FormComponent {
    ...
    constructor() {
        this.basicForm = new FormGroup({
            ...
            'birthdate': new FormControl(null, Validators.required, this.birthdateNotInFuture.bind(this)),
            'phone': new FormControl(null, [Validators.required, Validators.pattern('^[0-9]{9}$')]),
            // 'taxNumber': new FormControl(null, [Validators.required, Validators.pattern('^[1-3][0-9]{8}$')]),
            'taxNumber': new FormControl(null, [Validators.required, this.validateTaxNumber()]),
        });
    }
    ...
    validateTaxNumber(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            ...
            const sumAux = Array.from({ length: 8 }, (_, i) => 9 - i)
                .reduce((sum, digit, index) => sum + digit * parseInt(value.charAt(index), 10), 0);

            const module = sumAux % 11;
            const NIFwithoutLastDigit = value.slice(0, value.length - 1);

            if (module === 0 || module === 1) {
                return `${NIFwithoutLastDigit}0` === value ? null : { invalidTaxNumber: true };
            } else {
                return `${NIFwithoutLastDigit}${11 - module}` === value ? null : { invalidTaxNumber: true };
            }
        };
    }
